...
func TestGlobFileMatcher_MatchFiles(t *testing.T) {
	tempDir := t.TempDir()
	defer func() {
	...
	tests := []struct {
	...
	}{
		{
			...
			patterns: []string{"{{.markerName}}.rcd.gz", "{{.markerName}}.rcd_sig"},
			want: []string{
		...
		{
			name:     "valid sidecar files with two digits",
			patterns: []string{"sidecar/{{.markerName}}_0*.gz"},
			want: []string{
				fmt.Sprintf("%s/sidecar/test_marker_01.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_02.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_099.gz", tempDir),
			},
		},
		{
			name:     "valid sidecar files with any number of digits",
			patterns: []string{"sidecar/{{.markerName}}_[0-9]*.gz"},
			want: []string{
				fmt.Sprintf("%s/sidecar/test_marker_1.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_2.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_99.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_01.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_02.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_099.gz", tempDir),
			},
		...
		{
			name:     "valid sidecar files with one digit",
			patterns: []string{"sidecar/{{.markerName}}_[0-9].gz"},
			want: []string{
				fmt.Sprintf("%s/sidecar/test_marker_1.gz", tempDir),
				fmt.Sprintf("%s/sidecar/test_marker_2.gz", tempDir),
