...
func TestSidecarFileMatcher_MatchFiles(t *testing.T) {
	tempDir := t.TempDir()
	defer func() {
	...
	tests := []struct {
		...
		},
		{
			...
			patterns: []string{"sidecar/{{.markerName}}_##.gz"},
			want: []string{
		...
		},
		{
			...
			patterns: []string{"sidecar/{{.markerName}}_#.gz", "{{.markerName}}.rcd.gz"},
			want: []string{
				fmt.Sprintf("%s/test_marker.rcd.gz", tempDir),
